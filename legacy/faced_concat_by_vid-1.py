...
输入：
- EEG 段文件目录：${DAEST_DATA_ROOT}/data-faced/Cleaned_Data-3 下的 subXXX-YY_restored.set
- 播放顺序来源：${DAEST_DATA_ROOT}/data-faced/Data/subXXX/After_remarks.mat 中的 vid 字段
输出：
- 拼接后的 .set：${DAEST_DATA_ROOT}/data-faced/Cleaned_Data-4/subXXX_restored_concat.set
...
CLEANED_DIR = os.path.join(BASE_DIR, "Cleaned_Data-3")
RAW_DATA_DIR = os.path.join(BASE_DIR, "Data")
OUTPUT_DIR = os.path.join(BASE_DIR, "Cleaned_Data-4")
EEGLAB_DIR = os.environ.get("EEGLAB_DIR", "")

# 文件名模式：subNNN-YY_restored.set
SEGMENT_PATTERN = re.compile(r"^(sub\d+)-(\d+)_restored\.set$")
...
def collect_segments_for_subject(subject: str) -> Dict[int, str]:
    ...
    pattern = os.path.join(CLEANED_DIR, f"{subject}-*_restored.set")
...
def process_subject(subject: str) -> bool:
    ...
    try:
        ...
        if not ordered_files:
            ...
            return False
        out_path = os.path.join(OUTPUT_DIR, f"{subject}_restored_concat.set")
...
def main():
    ...
    if not os.path.isdir(CLEANED_DIR):
        logging.error("未找到 Cleaned_Data-3 目录：%s", CLEANED_DIR)
        return
    if not os.path.isdir(RAW_DATA_DIR):
