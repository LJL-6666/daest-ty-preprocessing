#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Build TY movie self-argmax 8-class input pkl for DAEST cross-subject.

Reuses EEG from output/9_movie/data_9class_movie.pkl; replaces stimulus labels
with per-subject per-video self-rating argmax (8 emotions, no neutral).

Does not overwrite 9_movie / 6_movie or any modeling results.
"""
...
def self_argmax_row(scores: np.ndarray) -> tuple[int, str, float, bool]:
    """Return (label_id, label_name, max_score, all8_zero).

    all8_zero / all-NaN → label_id=-1（无效样本，训练时按视频剔除，不映射为 anger）。
    """
    all_zero = bool(np.nansum(scores) == 0)
    if np.all(np.isnan(scores)) or all_zero:
        return -1, "", float("nan"), True if all_zero or np.all(np.isnan(scores)) else all_zero
...
def labels_for_subject(subject_id: str) -> tuple[np.ndarray, list[dict]]:
    ...
    if path is None:
        raise FileNotFoundError(f"subject {subject_id}: missing {EXP_PREFIX} rating csv")
    df = pd.read_csv(path)
    ...
    for i, vid in enumerate(VIDEO_IDS):
        if vid not in by_vid:
            raise ValueError(f"subject {subject_id}: missing videoIndex={vid} in {path}")
        info = by_vid[vid]
        labels[i] = int(info["self_argmax_id"])
        rows.append(info)
    return labels, rows


def majority_by_video(label_mat: np.ndarray) -> list[int]:
    """多数票仅统计有效标签（>=0）；若该视频全员无效则回退 0（仅供 ME metadata）。"""
    out = []
...
def write_readme(out_dir: Path, out_file: Path, subject_ids: list[str], data_shape: tuple, qc: dict) -> None:
    lines = [
        "# TY 观影任务 自评 argmax 8 类输入数据（DAEST 跨被试）",
        "",
        "本目录由 `extract_8class_movie_self.py` 从 `output/9_movie/data_9class_movie.pkl` + 问卷 `exp0` 生成。",
        "",
        "## 文件",
        "",
        f"- `{out_file.name}`：含 `data`、`subject_ids`、`label_by_subject_video`、`label_by_video`(多数票兜底)、`class_names`",
        "- `self_argmax_by_video.csv`：逐被试×视频自评标签与全 0 标记",
        ...
        "- 标签：自评 8 维情绪 argmax（无中性列）",
        "- **全 0 / 全 NaN 评分：标签=-1（无效），按视频剔除，不映射为 anger**",
        "",
        "## 8 类",
        "",
        "| id | name |",
    ...
    lines.append("| -1 | INVALID（全 0/无效，训练忽略） |")
    lines.extend(
        [
            "",
            "## 质控摘要",
            "",
            f"- 全 0 评分视频数（已标 -1 / 剔除）：{qc['n_all8_zero_videos']}",
            ...
            "## 注意",
            "",
            "- 不覆盖 `9_movie` / `6_movie` 与任何已有建模结果。",
            "- DAEST 跨被试 MLP 必须使用 `label_by_subject_video`（逐被试），不可 tile 多数票标签。",
            "- MLP / 指标使用 `ignore_index=-1`，全 0 视频不进损失与准确率。",
        ]
    )
...
def main() -> int:
    ...
    src_pkl = here / "output" / "9_movie" / "data_9class_movie.pkl"
    out_dir = here / "output" / "8_movie_self"
    out_file = out_dir / "data_8class_movie_self.pkl"
    ...
    n_all_zero = int(detail_df["all8_zero"].sum())
    n_sub_zero = int(detail_df.loc[detail_df["all8_zero"], "subject"].nunique())
        ...
        pkl.dump(
            {
                ...
                "label_source": "self_argmax_exp0",
                "n_class": 8,
                "invalid_label_id": -1,
                "invalid_policy": "drop_all8_zero_videos",
            },
            fp,
    ...
    print(f"invalid(-1) all8_zero: {n_all_zero}; subjects with any: {n_sub_zero}")
