#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Build the complete TY communication 9-class input pkl from preprocessed files.

Input:
  output/communication/*.pkl
  Supported shapes: (28, 31, 7500) or (32, 210000)
  Incomplete subjects (n_vids != 28) are skipped and logged.

Output:
  output/9_communication/data_9class_communication.pkl
  output/9_communication/README.md
  output/9_communication/skipped_files.tsv  (if any)

Does not touch output/6_communication or modeling results.
"""
...
# FACED / exp 素材标签顺序（与 extract_9class_movie.py 一致）
LABEL_9_BY_VIDEO = [
    0,
    0,
    0,
...
def write_readme(out_dir: Path, out_file: Path, subject_ids: list[str], data_shape: tuple[int, ...]) -> None:
    lines = [
        "# TY 交流任务 9 类完整输入数据",
        "",
        "本目录由 `extract_9class_communication.py` 从 `output/communication/*.pkl` 汇总生成。",
        "",
        "## 文件",
        "",
        ...
        "- 视频数：28（仅纳入完整 28 视频被试）",
        "- 通道数：31",
        "- 每视频点数：7500，即 30 秒 × 250 Hz",
        "- 标签体系：素材标签 9 类（与观影/FACED 视频 ID 映射一致）",
        "",
        "## 9 类标签映射",
        "",
    ...
    lines.extend(
        [
            ...
            "## 注意",
            "",
            "- 该文件保留 28 个视频，不同于 `output/6_communication/data_6class_communication.pkl` 的 19 视频/6 类版本。",
            "- 生成过程不覆盖已有 6 类数据和任何建模结果。",
            "- 视频数不足 28 的被试见 `skipped_files.tsv`。",
        ]
    )
...
def main() -> int:
    ...
    src_dir = here / "output" / "communication"
    out_dir = here / "output" / "9_communication"
    out_file = out_dir / "data_9class_communication.pkl"
    ...
    for path in files:
        try:
            ...
            if data is None:
                skipped.append((path.name, f"unsupported/incomplete shape {getattr(raw, 'shape', None)}"))
                continue
    ...
    if not data_blocks:
        raise RuntimeError("no valid communication pkl files could be loaded")
    ...
    if skipped:
        (out_dir / "skipped_files.tsv").write_text(
